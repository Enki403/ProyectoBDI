...
class DrawingApplication(tkinter.Frame):
    ...
    * Se crea las acciones sobre el dibujo y se define la funcion a ejecutar al interactuar con una determinada accion.
    ...
    def createDrawActions(self):
        ...
        self.fileMenu.add_command(label="Download",command=self.downloadFile)
        if(self.admin):
    ...
    def downloadFile(self):
        """
            * Descarga el contenido del dibujo en un .json y lo guarda en la carpeta de ejecucion del proyecto
        """
        if (self.currentDrawing != None):
            print('Hay un dibujo cargado: ', self.currentDrawing)
            name = ''
            cnx = mysql.connector.connect(**self.credentials)
            cursor = cnx.cursor()
            cursor.callproc('sp_getSketchDownload', [self.currentDrawing])
            binaryDrawing = None
            for result in cursor.stored_results():
                for row in result:
                    binaryDrawing = row[0]
                    name = row[1]
            #print('contenido a descargar: ', binaryDrawing)
            ...
            print('NO HAY UN DIBUJO CARGADO')
    ...
    def setCurrentDrawing(self, new):
        self.currentDrawing = new